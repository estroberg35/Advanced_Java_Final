...
public class PersonApplication extends JFrame implements ActionListener{
	// front end GUI for person application --> ability to create, save, load, edit, delete person objects with OCCC Date
	
	// TODO make sure to lock the save options if a person object is not completed.  This mostly works, come back and double check this
	
	// TODO When entering a date, if that date throws an OCCCDateException display a pop-up ("Invalid Date!") and clear the date field so that they can try again
	// sort of working, dates save but the exceptions are a bit weird and the pop up doesn't show up right
	
	// TODO make sure that if the application contains data that has changed from the most recent load, if the user clicks the 
	// closing X give them the usual pop up ("Save before exit?") so you won't accidentally throw away data.
	// i think this mostly works, keep testing
	
	
	JMenuItem fileMenu_new, fileMenu_open, fileMenu_save, fileMenu_saveAs, fileMenu_exit, helpMenu_about;
	...
	JPanel editPanel, instructionPanel, topPanel, personTypePanel, inputPanel, newInstructionPanel, typeButtonPanel, birthPanel;
	...
	public PersonApplication() {
		...
		// make the file menu and new, open, save, save as, and exit options
		...
		JLabel instructions = new JLabel("<html><div style='text-align: center;'>Please select person type, and then enter the corresponding information"
				...
				+ "<br>It prepares your list to be saved when you hit save!</div></html>");
	...
	public void updateInputFields() { // update the textbox inputs based on whether person, registeredperson, or occcperson is selected
		...
		if (newInstructionPanel != null) {
			this.remove(newInstructionPanel);
		}
